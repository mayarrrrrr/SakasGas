...
//             <Paper style={{ width: '100%', height:'100%', overflow: 'hidden', boxShadow: '10px 10px 15px 0px rgba(0,0,0,0.64)', background:'rgb(247,247,247)',  borderRadius:32,  backgroundColor: 'rgba(255, 255, 255, 0.7)' }}>
...
//                 component="div"
//                 count={adminOrders.length}  // Use 0 if adminOrders is undefined
...
function EnhancedTable() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      setAdminOrders(data); // Ensure data is in the expected format
    } catch (error) {
      console.error('Error fetching orders data:', error);
    }
  };
        ...
        <Paper sx={{ width: '100%', height: '100%', overflow: 'hidden' }}>
          <TableContainer sx={{ maxHeight: 440 }}>
            <Table stickyHeader aria-label="sticky table">
              ...
              <TableBody>
                {adminOrders.length === 0 ? ( // Check if there are any orders
                  <TableRow>
                  ...
                  adminOrders.orders.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((order) => (
                    <TableRow hover role="checkbox" tabIndex={-1} key={`${order.order_id}-${order.product_id}`}>
                      {columns.map((column) => {
                        const value = order[column.id];
                        return (
                          <TableCell key={column.id} align="center" style={column.id === 'status' ? { color: getStatusColor(value) } : {}}>
                            {value}
                          </TableCell>
                        );
                      })}
